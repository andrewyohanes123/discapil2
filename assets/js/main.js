...
{
  backendUrl = window.location.protocol + "//" + window.location.host + "/capil_dev/akte_mati/api";
}
else
{
    backendUrl = window.location.origin + "/capil_dev/akte_mati/api";
}
...
app.controller('no_kk', function($scope, $http, $cookies){
  var datas = $cookies.getObject('no_kk');
  if (!datas)
  {
    $scope.no_kk = "";
    $scope.nama_kepala_keluarga = "";
  }
  else
  {
